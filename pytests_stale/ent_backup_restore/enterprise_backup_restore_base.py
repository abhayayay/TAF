...
class EnterpriseBackupRestoreBase(BaseTestCase):
    def setUp(self):
        ...
        if "bin" not in bin_path:
            self.fail("Check if cb server install on %s" % self.cluster.master.ip)
        else:
        ...
        self.backup_validation_files_location = "/tmp/backuprestore" + self.cluster.master.ip
        ...
        self.cluster.master_services = self.cluster_util.get_services([self.backupset.cluster_host],
                                            self.services_init, start_node=0)
        if not self.cluster.master_services:
            self.cluster.master_services = ["kv"]
    ...
    def tearDown(self):
        ...
        if not self.input.param("skip_cleanup", False):
            ...
            validation_files_location = "%sbackuprestore" % self.tmp_path + self.cluster.master.ip
            if info == 'linux':
    ...
    def cluster_to_backup(self):
        return self.cluster_util.get_nodes_in_cluster(self.backupset.cluster_host)

    @property
    def cluster_to_restore(self):
        return self.cluster_util.get_nodes_in_cluster(self.backupset.restore_cluster_host)
    ...
    def _verify_backup_events_definition(self, bk_fxn):
        ...
        bk_file_events_dir = "/tmp/backup_events{0}/".format(self.cluster.master.ip)
...
class EnterpriseBackupMergeBase(EnterpriseBackupRestoreBase):
    ...
    def async_ops_on_buckets(self):
        ...
        """
        tasks = []
        create_tasks = self._async_load_all_buckets(self.cluster.master,
                                                    self.create_gen,
                                                    "create", self.expires)
        update_tasks = self._async_load_all_buckets(self.cluster.master,
                                                    self.update_gen,
                                                    "update", self.expires)
        delete_tasks = self._async_load_all_buckets(self.cluster.master,
                                                    self.delete_gen,
                                                    "delete", self.expires)
    ...
    def backup_with_expiry(self):
        ...
        """
        for bucket in self.buckets:
            cb = self._get_python_sdk_client(self.cluster.master.ip, bucket, self.backupset.cluster_host)
    ...
    def create_new_bucket_and_populate(self):
        ...
        for bucket in standard_buckets:
            self._load_bucket(bucket, self.cluster.master, self.initial_load_gen,
                              "create", self.expires)
    ...
    def delete_bucket(self):
        ...
        BucketOperationHelper.delete_bucket_or_assert(self.cluster.master,
                                                      bucket_to_delete,
                                                      self)
        self.buckets.pop()
    ...
    def delete_bucket_with_ops(self):
        ...
        BucketOperationHelper.delete_bucket_or_assert(self.cluster.master,
                                                      bucket_to_delete,
                                                      self)
        self.buckets = RestConnection(self.cluster.master).get_buckets()
        for task in ops_tasks:
            task.result()
    ...
    def load_flushed_buckets(self):
        ...
        for i in range(self.buckets.__len__() - self.bucket_to_flush,
                       self.buckets.__len__()):
            self._load_bucket(self.buckets[i], self.cluster.master,

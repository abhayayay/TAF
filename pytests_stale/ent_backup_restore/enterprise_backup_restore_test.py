...
class EnterpriseBackupRestoreTest(EnterpriseBackupRestoreBase, NewUpgradeBaseTest, EventingBaseTest):
    ...
    def test_cbbackupmgr_with_eventing(self):
        ...
        self.cluster.create_standard_bucket(name=self.src_bucket_name, port=self.standard_bucket_port+ 1,
                                                bucket_params=bucket_params)
        self.buckets = RestConnection(self.master).get_buckets()
        self.src_bucket = RestConnection(self.master).get_buckets()
        self.cluster.create_standard_bucket(name=self.dst_bucket_name, port=self.standard_bucket_port + 1,
                                                bucket_params=bucket_params)
        self.cluster.create_standard_bucket(name=self.metadata_bucket_name, port=self.standard_bucket_port + 1,
                                                bucket_params=bucket_params)
        self.buckets = RestConnection(self.master).get_buckets()

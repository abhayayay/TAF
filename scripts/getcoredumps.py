            ...
            if info.type.lower() != 'windows':
                ...
                for core_file in core_files:
                    if core_file.find('erl_crash.dump') != -1:
                        ...
                        command = "/opt/{" \
                                  "0}/bin/tools/cbanalyze-core".format(
                            server_type)
                        core_file_name = "core-{0}-{1}.log".format(
                            self.server.ip, i)
                        core_log_output = "/tmp/{0}".format(
                            core_file_name)
                        output, _ = remote.execute_command(
                            '{0} {1} -f {2}'.format(command, core_file,
                                                    core_log_output))
                        print(output)
                        remote_path, file_name = os.path.dirname(
                            core_log_output), os.path.basename(
                            core_log_output)
                        if remote.get_file(remote_path, file_name,
                                           os.path.join(self.path,
                                                        core_file_name)):
                ...
                if i > 0:
                    command = "mkdir -p /tmp/backup_crash/{0};" \
                              ...
                              "0};" \
                              "mv -f /opt/{1}/var/lib/{" \
                              "1}/erl_crash.dump* /tmp/backup_crash/{" \
                              "0}; " \
                              "mv -f /opt/{1}/var/lib/{1}/*.dmp " \
                              "/tmp/backup_crash/{0};" \
                              "mv -f /opt/{1}/var/lib/{1}/crash/*.dmp " \
                              "" \
                              "/tmp/backup_crash/{0};". \
...
def main():
    ...
    remotes = (Getcoredumps(server, file_path) for server in
               input.servers)

"""
Sirius client interface - Distributed workload
...
def clear_test_information(base_urls, identifier_token, username, password, bucket, scope=CbServer.default_scope,
    ...
    for base_url in base_urls.split("|"):
        ...
        for i in range(5):
            try:
                ...
                log.debug(("cleaning {0}".format(response_data)))
                if not response_data["error"]:
                    log.debug("{0}".format(response_data))
                return
            except Exception as e:
                log.warn((str(e)))
                exception = e
...
class SiriusClient(object):
    ...
    def send_request(self, path, method, payload=None, print_count=0):
        headers = {
            "Content-Type": "application/json"
        }
        ...
        if print_count == 0:
            self.log.debug("{0} {1}".format(url, payload))
        exception = None
        for i in range(5):
            try:
    ...
    def get_result_using_seed(self, response, load_type="bulk"):
        ...
        if response.status_code != 200:
            self.log.error(
                "(Bad/Malformed) operation {0} {1}".format(self.op_type, response.status_code))
            self.log.error("{0}".format(json.loads(response.content)))
            raise Exception("Bad HTTP status at doc loading {0}".format(response.status_code))
        ...
        if not result_from_request["error"]:
            ...
            self.log.error("error in initiating task : {0}".format(result_from_request["message"]))
            raise Exception(result_from_request["message"])
        ...
        for i in range(0, self.retry):
            if load_type == "bulk":
                ...
                time.sleep(0.5)
            response = self.send_request("/result", "POST", request_using_seed, int(i))
        ...
        if response.status_code != 200:
            self.log.error(
                "{0}/result {1} {2} {3}".format(self.sirius_base_url, request_using_seed, response.status_code,
                                                result_using_seed))

            self.log.error(
                ("unable to retrieve the result of operation : {0} {1}".format(self.op_type, response.status_code)))
            raise Exception(
                "unable to retrieve the result of operation : {0} {1}".format(self.op_type, response.status_code))

        if not result_using_seed["error"]:
            if "otherErrors" in list(result_using_seed["data"].keys()) and \
                    result_using_seed["data"]["otherErrors"] != "":
                if load_type == "warmup":
                    self.log.error(result_using_seed["data"]["otherErrors"])
                    raise Exception(result_using_seed["data"]["otherErrors"])
                self.log.warn(result_using_seed["data"]["otherErrors"])

            if load_type == "bulk":
                for error_name, failed_documents in list(result_using_seed["data"]["bulkErrors"].items()):
            ...
            self.log.error((result_using_seed["message"]))
    ...
    def do_bulk_sub_doc_operation(self, op_type, identifier_token=IDENTIFIER_TOKEN, insert_spec_options=None,
                                  remove_spec_options=None, replace_spec_options=None, get_spec_options=None,
                                  lookup_in_options=None, mutate_in_options=None, operation_config=None,
                                  retry=10, retry_interval=1, delete_record=False):

...
class RestConnection(newRC):
    ...
    def new_rebalance_status_and_progress(self, task_status_id=None):
        """
        ...
        api = self.baseUrl + "pools/default/tasks"
        status, content, header = self._http_request(api, timeout=60)
        json_parsed = json.loads(content)
    ...
    def extract_nodes_self_from_pools_default(self, pools_default):
        return next(iter([node for node in pools_default["nodes"] if "thisNode" in node and node["thisNode"]]), None)
    ...
    def set_AWS_bucket_credential_to_anlaytics(self, aws_access_key, aws_secret_key, aws_bucket_name,
        ...
        status_bucket_access, content, header = self._http_request(url, 'POST', urllib.parse.urlencode(data),
                                                                   headers=headers)

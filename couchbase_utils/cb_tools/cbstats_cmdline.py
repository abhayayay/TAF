...
class Cbstats(CbCmdBase):
    def __init__(self, server, username="Administrator",
                 password="password"):
        shell_conn = RemoteMachineShellConnection(server)
        CbCmdBase.__init__(self, shell_conn, "cbstats",
                           username=username, password=password)
        self.server = server
        # MB-47567 - Yet to support SSL ports in CbStats, using non-ssl port
        self.mc_port = CbServer.memcached_port

    @staticmethod
    def __calculate_vbucket_num(doc_key, total_vbuckets):
        """
